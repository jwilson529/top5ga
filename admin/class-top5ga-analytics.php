...
class Top5ga_Analytics {
    ...
    public function get_analytics_options() {
        ...
        $accounts_response = wp_remote_get( 'https://analyticsadmin.googleapis.com/v1beta/accounts', array(
        ...
        foreach ( $accounts_data['accounts'] as $account ) {
            ...
            $properties_response = wp_remote_get( 'https://analyticsadmin.googleapis.com/v1beta/properties?filter=parent:accounts/' . $account_id, array(
                'headers' => array( 'Authorization' => 'Bearer ' . $options['access_token'] ),
            ));
            ...
            foreach ( $properties_data['properties'] as $property ) {
                ...
                $streams_response = wp_remote_get( "https://analyticsadmin.googleapis.com/v1beta/properties/{$property_id}/dataStreams", array(
                    'headers' => array( 'Authorization' => 'Bearer ' . $options['access_token'] ),
                ));
        ...
        $response = wp_remote_post(
            ...
            array(
                ...
                'body' => json_encode(
                    array(
                        'dateRanges' => array(
                            array( 'startDate' => '30daysAgo', 'endDate' => 'today' ),
                        ),
                        'dimensions' => array(
                            array( 'name' => 'pagePath' ),
                        ),
